...
export const YourCampaignCard = ({ campaign }:{campaign: any}) => {
  return (
    <div className="group p-5 bg-[#161B22] border border-white/5 rounded-2xl hover:border-teal-500/30 transition-all duration-300 shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 text-gray-400">
          <IconUsers size={14} />
          <span className="text-[11px] font-medium">{campaign.age_ranges[0]}+</span>
        </div>
        <div className="flex items-center gap-2 text-gray-400">
          <IconCalendar size={14} />
          <span className="text-[11px] font-medium">{new Date(campaign.launch_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>
        </div>
      </div>
...
const YourCampaigns: React.FC<{Ctx: React.Context<BrandDashboardContextType> }> = ({ Ctx }) => {
  ...
  return (
    ...
    <div className="flex justify-between items-center mb-6">
          ...
          { !error && campaigns && campaigns.length > 5 ? (<button className="text-xs font-bold text-teal-400 hover:underline">View All</button>) : ''}
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {error && (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-rose-500/5 border border-rose-500/10 rounded-2xl">
            <IconAlertTriangle size={48} className="text-rose-500 mb-4 opacity-80" />
            <h4 className="text-white font-bold mb-1">Failed to load campaigns</h4>
            <p className="text-gray-500 text-xs max-w-[200px]">{error}</p>
            <button className="mt-4 text-xs font-bold text-rose-400 hover:underline">Retry Connection</button>
          </div>
        )}
