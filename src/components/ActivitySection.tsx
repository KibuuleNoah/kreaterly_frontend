...
import { IconCircleCheck, IconGitPullRequest, IconMessageDots, IconFlame } from '@tabler/icons-react';

const activities = [
  {
    id: 1,
    title: "Project Milestone Reached",
    desc: "The initial beta version of the dashboard was deployed to the staging environment successfully.",
    when: "2 hours ago",
  ...
  },
  {
    ...
    desc: "Alex submitted a PR: 'feat: add dark mode support using Tailwind CSS strategy'.",
    when: "5 hours ago",
    icon: <IconGitPullRequest className="text-blue-500" size={20} />,
    color: "bg-blue-100"
  },
  {
    id: 3,
    title: "Team Meeting",
    desc: "Discussed the Q3 roadmap and finalized the component library migration plan.",
    when: "Yesterday",
    icon: <IconMessageDots className="text-purple-500" size={20} />,
    color: "bg-purple-100"
  }
];
  ...
  return (
    ...
    <section className="w-full max-w-4xl p-8 bg-[#0D1117] rounded-3xl border border-white/5 shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-10">
        ...
        <h2 className="text-xl font-bold text-white tracking-tight">Recent Activities</h2>
      </div>
      ...
      <div className="space-y-10 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-[2px] before:bg-gradient-to-b before:from-transparent before:via-teal-500/20 before:to-transparent">
        {activities.map((item) => (
          <div key={item.id} className="relative pl-12 group">
            ...
            <div className={`absolute left-0 p-2 rounded-xl border border-white/10 bg-[#161B22] shadow-xl transition-all duration-300 group-hover:border-teal-500/50 group-hover:scale-110 ${item.color}`}>
              {item.icon}
            </div>
